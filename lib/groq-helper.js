...
class GroqClient {
  ...
  validateConfig() {
    if (!this.apiKey) {
      throw new Error(
        "❌ GROQ_API_KEY non définie dans les variables d'environnement",
      );
    }
    if (!this.model) {
  ...
  async generateCode(prompt, context = "", maxRetries = 2) {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        console.warn(
          `⚠️ Tentative ${attempt}/${maxRetries} échouée, réessai...`,
        );
          ...
          messages: [
            {
              role: "system",
              content:
                "Tu es un assistant codeur expert. Réponds UNIQUEMENT au format JSON demandé.",
            },
            {
              role: "user",
              content: prompt,
