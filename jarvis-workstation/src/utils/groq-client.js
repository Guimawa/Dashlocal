...
class GroqClient {
     ...
     */
    async sendRequest(messages, model = GROQ_CONFIG.defaultModel, temperature = 0.7) {
        ...
        for (let attempt = 1; attempt <= GROQ_CONFIG.maxRetries; attempt++) {
            try {
                ...
                if (error.message.includes('model') && GROQ_CONFIG.fallbackModels.length > 0) {
...
- Optimisé pour les performances`
    ...
    getMetrics() {
        return {
            ...this.metrics,
            averageTime: this.metrics.successfulRequests > 0 
