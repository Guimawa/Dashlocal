...
class JarvisBrain {
  ...
  async understandRequest(prompt, context) {
    ...
    const enrichedAnalysis = await this.memory.enrichWithPastGenerations(analysis);
  ...
  async loadProjectContext(analysis) {
    ...
    this.projectState = await this.memory.getProjectState(projectType, techStack);
  ...
  async executeGeneration(generationPlan) {
    ...
    if (generationPlan.components.length > 0) {
      try {
        const { default: ReactGenerator } = await import('../generators/react-gen.js');
        const reactGen = new ReactGenerator();
        for (const component of generationPlan.components) {
          try {
            const componentCode = await reactGen.generateComponent(component, this.projectState);
            ...
            results.errors.push(`Composant ${component.name}: ${error.message}`);
          }
        }
      } catch (error) {
  ...
  async integrateToProject(validatedResults) {
    ...
    for (const component of validatedResults.components) {
      try {
        const filePath = await this.writeToProject(component.path, component.code);
  ...
  async analyzeRequirements(prompt, context) {
    try {
      const messages = [
        {
          role: "system",
          content: "Tu es un expert en analyse de spécifications techniques. Analyse cette demande et retourne une structure JSON détaillée."
        },
        {
          role: "user",
      ...
      console.warn('Erreur analyse IA, utilisation de l\'analyse par défaut:', error.message);
      return {
        type: 'component',
        complexity: 'medium',
        domain: 'react',
  ...
  async generateDocumentation(results) {
    // Implémentation simplifiée
    console.log('📚 Génération documentation pour', results.added.length, 'fichiers');
  }
}
