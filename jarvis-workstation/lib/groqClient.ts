...
export function generatePrompt(action: string, context?: string): string {
  const basePrompts = {
    ...
    'refactor-code': 'Refactor this React code for better performance and maintainability',
    ...
    'convert-to-hooks': 'Convert this class component to functional component with hooks',
    'add-accessibility': 'Add accessibility features to this React component',
    'responsive-design': 'Make this React component responsive for mobile and desktop'
  };

  const basePrompt = basePrompts[action as keyof typeof basePrompts] || 'Generate React code';
...
export function createSystemPrompt(componentType: string = 'component'): string {
...
export function formatCodeForPreview(code: string): string {
  ...
  formattedCode = formattedCode.replace(/```(?:jsx?|tsx?|javascript|typescript)?\n?/g, '');
