        ...
        const answers = await inquirer.prompt([
          {
            ...
            validate: (input) =>
              input ? true : "Le nom ne peut pas être vide",
          },
        ]);
    ...
    const answers = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input ? true : "La question ne peut pas être vide",
      },
    ]);
...
async function teachMode(question) {
  console.log(
    chalk.blue(`🤔 Jarvis réfléchit à votre question: "${question}"`),
  );
  ...
  const messages = [
    {
      role: "system",
      content:
        "Tu es Jarvis, un assistant IA expert en développement. Explique clairement et de manière pédagogique.",
    },
    {
      role: "user",
      content: question,
